find_markers_specific_clusters <- function(seu_path,
  ...
  if(test.use == "DESeq2") {
    ...
    find_markers <- FindMarkers(pseudo,
  ...
  if(!is.null(annotate_peaks))  {
    ...
    saveRDS(da_cr, file.path(save.loc,paste0("FindMarkers.",output_name,".",idents.1,"_",paste0(idents.2, collapse = "_"),".annotated_dacr.rds")))
  } else {
    
    message("[MSG] saving FindMarkers results in ",output_name,"...")
    saveRDS(find_markers ,file.path(save.loc, paste0("FindMarkers.",output_name,".",idents.1,"_",paste0(idents.2, collapse = "_"),".rds")))
