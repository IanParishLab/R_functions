...
preprocess_flex_data <- function(
    project.loc,
    species = "human",
    seed.use, min.pc, bk.list = NULL, vars.to.regress = "percent.mito",
    save.loc = project.loc, dry_run = TRUE,
    overwrite = FALSE, parallel = FALSE, workers = 4, 
    steps = list(decontX = FALSE, normalise = FALSE,
  ...
  if (!steps$normalise) {
    ...
    so <- lapply(seq_along(capture), function(i) {
      ...
      sce_filt <- run_scaterQC(
        sce = sce,
        nmads = c(low = 2, high = 2), # high=3 in fresh scrnaseq - 2 is cleaner? hopefully
  ...
  if (!steps$filtering_singlets) {
    ...
    message("[INFO] Filtering scDblFinder doublets...")
    ...
    lapply(seq_along(capture), function(i) {
      ...
      qs_save(so[[capture[i]]], here(save.loc, "2_QC","int_obj", paste0(so[[capture[i]]]@project.name, ".filtered.qs")))
    })
  }
  ...
  if (!steps$cluster_qc) {
    ...
    if (!exists("so") || is.null(so) || length(so) == 0) {
      so <- load_so_from_stage(capture, "2_QC", save.loc, pattern = "filtered")
    }
    ...
    so <- lapply(seq_along(capture), function(i) {
      ...
      seu <- find_clusters_to_remove(
        ...
        seed.use = seed.use, prefix = "0", save.loc = here(save.loc, "3_cluster_qc_1"),
    ...
    lapply(seq_along(capture), function(i) {
      ...
      qs_save(so[[capture[i]]], here(save.loc, "3_cluster_qc_1", "int_obj", paste0(so[[capture[i]]]@project.name, ".clustered.qs")))
    })
  }
